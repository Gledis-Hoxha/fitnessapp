...
function MiniStatCard({ icon: Icon, label, value, color }) {
  return (
    <div className="bg-[#111] border border-white/8 rounded-xl px-3 py-2 flex items-center gap-2.5">
      <div className={`p-1.5 rounded-lg flex-shrink-0 ${color}`}>
        <Icon className="w-3 h-3" />
      </div>
      <div className="min-w-0">
        <p className="text-sm font-bold text-white leading-none">{value}</p>
        <p className="text-[10px] text-white/35 font-medium mt-0.5">{label}</p>
      </div>
    </div>
  );
}
    ...
    <div className="space-y-4 pb-4">
      ...
      <div className="space-y-1.5">
        <div className="flex items-baseline justify-between">
          <p className="text-[10px] font-semibold text-white/35 uppercase tracking-widest">Today's Overview</p>
          <p className="text-[10px] text-white/20">{format(new Date(), "EEE, MMM d")}</p>
        </div>
